...
import { assert, stringToU8a, u8aToString } from '@axia-js/util';
...
export function withWasm <T, F extends (...params: never[]) => T> (fn: (wasm: WasmCryptoInstance) => F): F {
  return ((...params: never[]): T => {
    assert(wasm, 'The WASM interface has not been initialized. Ensure that you wait for the initialization Promise with waitReady() from @axia-js/wasm-crypto (or cryptoWaitReady() from @axia-js/util-crypto) before attempting to use WASM-only interfaces.');
